...
const supabase = createClient(
  'https://vtnykubyupjahoalarba.supabase.co',
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZ0bnlrdWJ5dXBqYWhvYWxhcmJhIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2MTUwMjgwOCwiZXhwIjoyMDc3MDc4ODA4fQ.bKrP_w7yh6tHCuI7L0R_2lqVDU1X0XG1t7v8Z5C0cKs'
);

// Update process_recurring_transactions function
const updateProcessRecurringSQL = `
...
// Update create_reminder_notifications function
...
async function updateFunctions() {
  try {
    console.log('Updating process_recurring_transactions function...');
    const result1 = await supabase.rpc('exec_sql', { 
      sql: updateProcessRecurringSQL 
    });
    
    if (result1.error) {
      console.error('Error updating process_recurring_transactions:', result1.error);
    } else {
      console.log('✓ process_recurring_transactions updated');
    }

    console.log('\nUpdating create_reminder_notifications function...');
    const result2 = await supabase.rpc('exec_sql', { 
      sql: updateReminderSQL 
    });
    
    if (result2.error) {
      console.error('Error updating create_reminder_notifications:', result2.error);
    } else {
      console.log('✓ create_reminder_notifications updated');
    }

    console.log('\n✓ All functions updated successfully!');
  } catch (error) {
    console.error('Unexpected error:', error);
    console.log('\nPlease manually execute the SQL in Supabase:');
    console.log('1. Go to: https://app.supabase.com/project/vtnykubyupjahoalarba/sql/new');
    console.log('2. Paste the SQL from scripts/recurring-transactions.sql');
