...
export function BadgeDisplay({
  ...
  return (
    <div
      className={`inline-flex items-center rounded-full bg-gradient-to-r from-slate-900/50 to-slate-800/50 border border-slate-700/50 backdrop-blur-sm ${sizeClasses[size]} ${glowClasses} ${animationClasses}`}
      style={{
...
export function BadgeGroup({
  ...
  return (
    <div className={`flex flex-wrap gap-2 ${className}`}>
      ...
      {remainingCount > 0 && (
        <div
          className={`inline-flex items-center rounded-full bg-slate-900/50 border border-slate-700/50 px-3 py-1.5 text-sm font-semibold text-slate-300 hover:text-white transition-colors`}
        >
          +{remainingCount}
        </div>
      )}
